...
//            std::cout << "Reading input: " << entry.path().u8string() << std::endl;
...
bool is_lines_of_slope_near_to_0_or_90degree(double theta) {
...
bool is_lines_horizontal(double theta) {
    if (abs(theta) < 45)
        return true;
    else
        return false;
...
std::vector<std::vector<double>> FilterLines(std::vector<cv::Vec4i> Lines)
{
    ...
    for (int i = 0; i < Lines.size(); i++)
    {
        ...
        // Rejecting lines of slope near to 0 degree or 90 degree and storing others
        // And only pick horizontal-looking lines
        if (!is_lines_of_slope_near_to_0_or_90degree(theta) && is_lines_horizontal(theta))
        {
//            printf("(%d, %d), (%d, %d) - theta: %lf\n", x1, y1, x2, y2, theta);
...
int demo_vanishing_point()
{
    ...
    ReadImage("/Users/3i-21-331/workspace/CppPractive/CppPractive/img/pitch_positive", Images, ImageNames);
    ...
    for (int i = 0; i < Images.size(); i++)
    {
        ...
        std::cout << "Vanishing Point: (" << VanishingPoint[0] << ", " << VanishingPoint[1] << ")" << std::endl;
        cv::circle(Image, cv::Point(VanishingPoint[0], VanishingPoint[1]), 20, cv::Scalar(0, 0, 255), -1);
        float tilt_angle = atan2(VanishingPoint[1] - 2000, 4000) * 180 / M_PI;
        printf("tilt angle: %f \n", tilt_angle);
